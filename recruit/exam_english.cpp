...
QuestionList CreatEnglishWordExam()
{
	...
	case 1: // 「読み」から英単語を答える
		for (int i = 0; i < quizCount; i++) {
			const auto& e = data[indices[i]];
			questions.push_back({
					"カタカナ読み「" + string(e.reading) + "」をに対応する英単語を答えよ",
					e.word });
		}
		break;
...
QuestionList CreateEnglishPhraseExam()
{
	static const struct {
		...
		int blankOption[3];		//穴の候補になる単語の位置
	} data[] = {
		...
		"Mom says that you can have either a dog or a cat", "母は、犬か猫のどちらかなら買っていいと言っている。",
		2, { 6, 9 }},
		...
		{"take care of [A]", "[A]の世話をする",
			"she always takes care of her sister.", "彼女じゃいつも妹の面倒を見ている。",
		3, {3, 4, 5 }},
	};
	...
	case 0: // 空欄に入る単語を答える
		for (int i = 0; i < quizCount; i++) {
			...
			int blankIndex = e.blankOption[n];
